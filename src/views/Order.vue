  ...
  <div>
    <div style="margin-bottom: 30px">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>发货指令管理</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    <div style="margin: 10px 0">
      ...
      <el-select  v-model="type" placeholder="请选择发货指令状态" class="ml-5" >
        <el-option
    ...
    <div style="margin: 10px 0">
      ...
      <el-upload action="http://localhost:9090/port-order/import" :show-file-list="false" accept="xlsx" :on-success="handleExcelImportSuccess" style="display: inline-block">
    ...
    <el-table :data="tableData" border stripe header-cell-class-name="headerBg"  @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="type" label="发货指令状态" ></el-table-column>
    ...
    <el-dialog title="新增发货指令信息" :visible.sync="dialogFormVisible" width="30%">
      <el-form label-width="70px" size="small" :model="order" :rules="rules" ref="orderForm">
        <el-form-item label="申请人" prop="client">
          <el-input v-model="order.client" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="船舶名" prop="shipName">
          <el-input v-model="order.shipName" autocomplete="off"></el-input>
        </el-form-item>
...
<!--        </el-form-item>-->
    ...
    <el-dialog title="审批发货指令信息" :visible.sync="EditFormVisible" width="30%">
  ...
  methods:{
    ...
    save(){
      this.$refs['orderForm'].validate((valid)=>{
        if(valid){
          this.order.company=this.user.company
          this.order.type='未办'
          this.request.post("/port-order",this.order).then(res =>{
            if(res){
    ...
    Edit(){
      ...
      this.request.post("/port-order",this.form).then(res =>{
        if(res){
    ...
    handleDelete(id){
      this.request.delete("/port-order/"+id).then(res =>{
        if(res) {
    ...
    load(){
      ...
      this.request.get("/port-order/page",{
            params:{
    ...
    handleAdd(){
      ...
      this.order={}
      this.order.client=this.user.username
    ...
    delBatch(){
      ...
      this.request.post("/port-order/del/batch",ids).then(res =>{
        if(res) {
    ...
    exp(){
      ...
      window.open("http://localhost:9090/port-order/export/"+this.user.company)
    },
    getCurrentTime() {
      //获取当前时间并打印
